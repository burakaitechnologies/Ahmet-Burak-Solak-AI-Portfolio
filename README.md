# Ahmet Burak Solak - Applied AI Portfolio

## About Me

I am an Applied AI AAS student in ITAI-1378, focused on computer vision, practical AI systems, and beginner-friendly applied machine learning workflows. This portfolio collects my computer vision labs, reflections, notebooks, and project proposal work from the course.

## Program

- Student: Ahmet Burak Solak
- Program/Course: ITAI-1378 AI AAS
- GitHub: https://github.com/burakaitechnologies
- Expected repository URL: https://github.com/burakaitechnologies/Ahmet-Burak-Solak-AI-Portfolio
- LinkedIn: https://www.linkedin.com/in/ahmet-burak-s-582676204/
- Email: 145400756+burak626@users.noreply.github.com

## Technical Skills

- Python, Jupyter Notebook, Google Colab
- OpenCV, Pillow, NumPy, Matplotlib
- Image processing and digital image representation
- Classical machine learning for computer vision
- CNNs and transfer learning
- Object detection, object tracking, and zone analytics
- Visual language model concepts
- AI agents, tools, memory, and reasoning loops

## Featured Course Work

### Computer Vision - ITAI 1378

This course portfolio includes hands-on labs and written reflections covering the full path from basic pixels to applied AI vision systems.

- [Course Folder](ComputerVision-ITAI1378/)
- [Course Projects Summary](CourseProjects.md)
- [Plant Disease Detection Proposal](ComputerVision-ITAI1378/Midterm-Plant-Disease-Proposal/)
- [Object Tracking and Zone Analytics](ComputerVision-ITAI1378/L11-Object-Tracking-Zone-Analytics/)
- [Visual Language Models](ComputerVision-ITAI1378/L08-Visual-Language-Models/)
- [Object Detection with Transfer Learning](ComputerVision-ITAI1378/L06-Object-Detection-Transfer-Learning/)
- [CNN Image Classification](ComputerVision-ITAI1378/L05-CNN-Chihuahua-vs-Muffin/)

## Repository Organization

Each project folder contains the original notebook, report, slide deck, or reflection artifact when available. Project README files explain the problem, approach, results, tools, and how to run or review the work.

## Contact

- GitHub: https://github.com/burakaitechnologies
- LinkedIn: https://www.linkedin.com/in/ahmet-burak-s-582676204/
- Email: 145400756+burak626@users.noreply.github.com
